...
class AEalFlowNoiseTemplate(alShadersTemplate):
	...
	def setup(self):
		self.params.clear()
		self.params["space"] = Param("space", "Space", "Space in which to calculate the noise pattern. For best results, use UV.", "enum", presets=None)
		...
		self.params["angle"] = Param("angle", "Angle", "Animate this to control the rolling motion of the noise.", "float", presets=None)
		self.params["advection"] = Param("advection", "Advection", "Controls how much the smaller noise features are swept along with the larger ones.", "float", presets=None)
		...
		self.addCustomFlt("angle")
		self.addCustomFlt("advection")
