...
def create_node(this_node=''):
    ...
    renderPath = os.path.join((nuke.root().name()) ,"../..") + "/{render}/{file}/exr/{file}.%04d.exr".format(render=Tank().data['rules']['STATUS']['render'], file=fileName)
...
def render():
    ...
    note = arNotice.Notice(title = os.path.basename(nuke.root().name()).split('.')[0],
                           ...
                           func  = tmp_job,
                           img   = 'lbl/nuke' if tmp_job!='NO JOB' else 'lbl/lblWarning131',
                           img_link = '')
