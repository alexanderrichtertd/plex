...
class ArUtil(object):
    ...
    def add_preview(self, layout):
        ...
        self.wgPreview = QtUiTools.QUiLoader().load(path_ui)
    ...
    def add_menu(self):
        ...
        self.wgMenu = QtUiTools.QUiLoader().load(path_ui)
    ...
    def press_btnPreviewImg(self):
        if os.path.exists(self.preview_img_path): webbrowser.open(self.preview_img_path)
        # self.img_path = self.folder_msg_box(self.wgHeader, "Image Files (*.jpg *.png *.tif)",
        #                                     "Choose image file", os.environ['USERPROFILE'] + "/Desktop")
        # if self.img_path: self.wgPreview.btnPreviewImg.setIcon(QtGui.QPixmap(QtGui.QImage(self.img_path)))
