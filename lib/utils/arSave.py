...
class ArSave(ArUtil):
    ...
    def __init__(self, parent=None):
        ...
        self.wgSave = QtUiTools.QUiLoader().load(path_ui)

        self.save_dir  = os.getenv('PROJECT_PATH')
        self.save_file = ''
        self.comment   = "Comment"
        self.img_path  = libFileFolder.rm_tmp_img()
    ...
    def set_path(self):
        ...
        libSnapshot.create_any_screenshot(self.wgSave, self.wgSave.btnPreviewImg)
    ...
    def update_version(self, add=1):
        ...
        if found_version:
            ...
            self.save_file = os.path.dirname(self.save_file) + '/' + os.path.basename(self.save_file).replace(old_version, new_version)
    ...
    def save_file_path(self):
        ...
        try:
            filePath  = Tank().software.scene_saveAs(self.save_file)
            self.set_meta_data()
        ...
        if self.wgHeader.cbxAdd.isChecked():
            ...
            libFileFolder.create_folder(os.path.dirname(self.save_publish_file))
