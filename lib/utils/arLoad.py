...
class ArLoad(ArUtil):
    ...
    def __init__(self):
        ...
        self.wgLoad = QtUiTools.QUiLoader().load(path_ui)
        ...
        self.wgLoad.lstScene.connect(self.wgLoad.lstScene, QtCore.SIGNAL("itemSelectionChanged()"), self.change_lstScene)
        self.wgLoad.lstSet.connect(self.wgLoad.lstSet, QtCore.SIGNAL("itemSelectionChanged()"), self.change_lstSet)
        self.wgLoad.lstAsset.connect(self.wgLoad.lstAsset, QtCore.SIGNAL("itemSelectionChanged()"), self.change_lstAsset)
        self.wgLoad.lstTask.connect(self.wgLoad.lstTask, QtCore.SIGNAL("itemSelectionChanged()"), self.change_lstTask)
        self.wgLoad.lstStatus.connect(self.wgLoad.lstStatus, QtCore.SIGNAL("itemSelectionChanged()"), self.change_lstStatus)
        self.wgLoad.lstFiles.connect(self.wgLoad.lstFiles, QtCore.SIGNAL("itemSelectionChanged()"), self.change_lstFiles)

        self.wgHeader.btnOption.clicked.connect(self.press_menuItemAddFolder)
        self.wgLoad.lstFiles.connect(self.wgLoad.lstFiles,QtCore.SIGNAL("itemDoubleClicked(QListWidgetItem *)"), self.press_btnAccept)

        self.wgLoad.lstScene.setContextMenuPolicy(QtCore.Qt.CustomContextMenu)
        self.wgLoad.lstScene.connect(self.wgLoad.lstScene,QtCore.SIGNAL("customContextMenuRequested(QPoint)"), lambda: self.press_openMenu(self.wgLoad.lstScene))
        self.wgLoad.lstSet.setContextMenuPolicy(QtCore.Qt.CustomContextMenu)
        self.wgLoad.lstSet.connect(self.wgLoad.lstSet,QtCore.SIGNAL("customContextMenuRequested(QPoint)"), lambda: self.press_openMenu(self.wgLoad.lstSet))
        self.wgLoad.lstAsset.setContextMenuPolicy(QtCore.Qt.CustomContextMenu)
        self.wgLoad.lstAsset.connect(self.wgLoad.lstAsset,QtCore.SIGNAL("customContextMenuRequested(QPoint)"), lambda: self.press_openMenu(self.wgLoad.lstAsset))
        self.wgLoad.lstTask.setContextMenuPolicy(QtCore.Qt.CustomContextMenu)
        self.wgLoad.lstTask.connect(self.wgLoad.lstTask,QtCore.SIGNAL("customContextMenuRequested(QPoint)"), lambda: self.press_openMenu(self.wgLoad.lstTask))
        self.wgLoad.lstFiles.setContextMenuPolicy(QtCore.Qt.CustomContextMenu)
        self.wgLoad.lstFiles.connect(self.wgLoad.lstFiles,QtCore.SIGNAL("customContextMenuRequested(QPoint)"), lambda: self.press_openMenu(self.wgLoad.lstFiles))
        ...
        self.wgHeader.setWindowIcon(QtGui.QIcon(libData.get_img_path("btn/btnFolderSearchGet48")))
    ...
    def press_btnAccept(self):
       ...
       # OPEN in current software
        ...
        if open_software in self.data['software'].keys():
        ...
        note = arNotice.Notice(title = os.path.basename(self.load_file).split('.')[0],
                               ...
                               func  = 'LOAD',
                               img   = self.preview_img_path,
                               img_link = os.path.dirname(self.load_file))
        arNotice.ArNotice(note)

    def press_openMenu(self, list_widget):
        self.listMenu = QtGui.QMenu()
        self.listMenu.setStyleSheet(self.data['style']['arDesktop']['menu'])
        menu_item = self.listMenu.addAction("Add " + self.wgLoad.lstScene.currentItem().text())
        self.wgLoad.connect(menu_item, QtCore.SIGNAL("triggered()"), self.press_menuItemAddFolder)
        menu_item = self.listMenu.addSeparator()
        menu_item = self.listMenu.addAction("ABC")
        self.wgLoad.connect(menu_item, QtCore.SIGNAL("triggered()"), lambda: self.press_menuSort(list_widget))
        menu_item = self.listMenu.addAction("CBA")
        self.wgLoad.connect(menu_item, QtCore.SIGNAL("triggered()"), lambda: self.press_menuSort(list_widget, True))
    ...
    def change_lstScene(self):
        ...
        tmp_content   = libFileFolder.get_file_list(self.load_dir)
    ...
    def change_lstSet(self):
        ...
        tmp_content = libFileFolder.get_file_list(new_path)
    ...
    def change_lstStatus(self):
        ...
        tmp_content   = libFileFolder.get_file_list(self.file_dir, extension=True)
