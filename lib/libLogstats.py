...
USER_STATS_PATH = "{}/{}.stats".format(os.getenv('DATA_USER_PATH'), libLog.USER)


#*********************************************************************
# FUNC
def notice_func(script_string, meta=False, notice=True):
        ...
        def wrapper(*args, **kwargs):
            ...
            if note:
                if ":" in str(result_func):
                    plexnotice.create_default_notice(result_func)
                elif type(result_func) == str:
                    plexnotice.create_default_notice("common:fail", result_func)
                else:
                    plexnotice.create_default_notice(script_string)
...
# START
def example():
    @notice_func("shelf/open scene", True)
    def echo(foo):
        time.sleep(0.2)
        print("DONE     - echo")
